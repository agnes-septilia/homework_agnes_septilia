...
host='localhost'
port='5432'
zip_medium_file = '/home/agnes/Documents/digital_skola/Project/project_3_de/temp/dataset-medium.zip'
medium_file_name = 'dataset-medium.csv'
table_name = 'user_address_2018_snapshots_2'
...
df_chunk = pd.read_csv(zf.open(medium_file_name), header=None, chunksize = chunk_size)


## assign the task in looping program
for i, chunk in enumerate(df_chunk):
    ## assign column 
    chunk.columns = col_name_df 
    ...
    print(f'Page {i + 1} is uploaded!') 


## print the result
print(f'Total inserted rows: {len(df)}')
print(f'Last created_at: {df.created_at.max()}')

row_number = df[df['created_at'] == df.created_at.max()].index[0]
if row_number % chunk_size > 0:
    page = (row_number // chunk_size) + 1
else:
    page = (row_number // chunk_size)
print(f'From data part: {page}')
